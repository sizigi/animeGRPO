...
# v5-utmos: 1B-Multilingual + UTMOS objective + CER hard-constraint + adaptive KL
# ----------------------------------------------------------------------------
# Ablation of run_jp_grpo_animescore_constrained_v5.sh — only difference is
# AnimeScore swapped for UTMOS as the reward axis. All other knobs identical:
#   - adaptive KL (target_kl=0.05, kl_coef=0.05, horizon=2000)
#   - CER hard constraint (≤0.30 feasible, ≤0.10 clean, >0.30 violate=-1.0)
#   - top_p=0.85, repetition_penalty=1.05, n=4, gpu_mem=0.6
#   - launch_reward_fn_async=True, total_epochs=40
# ----------------------------------------------------------------------------

export UTMOS_SERVER=http://localhost:8007
...
CUDA_VISIBLE_DEVICES=0 python3 -m verl.trainer.main_ppo \
    ...
    custom_reward_function.path=verl/utils/reward_score/tts_utmos_cer_constraint.py \
    custom_reward_function.name=compute_score \
    reward_model.launch_reward_fn_async=True \
    trainer.project_name='llasa_1b_grpo_v5_utmos_constrained' \
    trainer.experiment_name='utmos_cer_constraint_adaptive_kl' \
