...
# v5-vad-arousal: 1B-Multilingual + VAD Arousal objective + CER hard-constraint + adaptive KL
# ----------------------------------------------------------------------------
# Ablation of run_jp_grpo_utmos_constrained_v5.sh — only difference is
# UTMOS swapped for VAD Arousal as the reward axis. All other knobs identical.
# ----------------------------------------------------------------------------

export VAD_SERVER=http://localhost:8006
...
CUDA_VISIBLE_DEVICES=0 python3 -m verl.trainer.main_ppo \
    ...
    custom_reward_function.path=verl/utils/reward_score/tts_vad_arousal_cer_constraint.py \
    custom_reward_function.name=compute_score \
    reward_model.launch_reward_fn_async=True \
    trainer.project_name='llasa_1b_grpo_v5_vad_arousal_constrained' \
    trainer.experiment_name='vad_arousal_cer_constraint_adaptive_kl' \
