...
ROOT = Path("~/samples/llasa_1b_v6_step540_eval").expanduser()
...
V6_CKPT = os.path.expanduser("~/checkpoints/llasa-1b-v6-step540-merged")
...
def main():
    ...
    print(f"Loading v4 ckpt {V6_CKPT}")
    tokenizer = AutoTokenizer.from_pretrained(V6_CKPT)
    model = AutoModelForCausalLM.from_pretrained(V6_CKPT, torch_dtype=torch.bfloat16).to(DEVICE).eval()
    ...
    clean_dir = ROOT / "clean_v6_540"; clean_dir.mkdir(exist_ok=True)
    chosen = {}
    for r in rows:
        i = r["idx"]
        seed0 = raw[i]
    ...
    for r in rows:
        ...
        per_sample.append({
            ...
            "base": {"seed": b.get("seed"), "cer": b.get("cer"),
                     ...
                     "utmos": b.get("utmos")},
            "v6_540_raw_seed0": {"cer": rr.get("cer"),
                                 "animescore": rr.get("animescore"),
                                 "utmos": rr.get("utmos")},
            "v6_540_clean":    {"seed": v.get("seed"), "cer": v.get("cer"),
    ...
    raw_list  = [p["v6_540_raw_seed0"] for p in per_sample]
    cln_list  = [p["v6_540_clean"]     for p in per_sample]

    summary = {
        "n": NUM_EVAL,
        "ckpt": V6_CKPT,
        "reward": "tts_animescore.py (raw AS, no CER)",
        "cer_threshold": CER_THRESHOLD,
        "base_summary":     {k: stat(base_list, k) for k in ("cer","animescore","utmos","likability")},
        "v6_raw_seed0":     {k: stat(raw_list,  k) for k in ("cer","animescore","utmos")},
        "v6_clean":         {k: stat(cln_list,  k) for k in ("cer","animescore","utmos")},
    ...
    deltas_AS    = [p["v6_540_clean"].get("animescore",0) - p["base"].get("animescore",0)
                    for p in per_sample
                    if p["v6_540_clean"].get("animescore") is not None and p["base"].get("animescore") is not None]
    deltas_UTMOS = [p["v6_540_clean"].get("utmos",0) - p["base"].get("utmos",0)
                    for p in per_sample
                    if p["v6_540_clean"].get("utmos") is not None and p["base"].get("utmos") is not None]
    summary["delta_clean"] = {
    ...
    print(json.dumps({k: summary[k] for k in
                      ("base_summary","v6_raw_seed0","v6_clean","violation_rate_seed0","delta_clean")},
